...
(function() {
  ...
  program.command('set <name>').description('output one page of cards from the named set').option('-f, --format [formatter]', '"json" or "summary"', 'summary').option('-p, --page [number]', 'specify page number', 1).action(function(name, options) {
    return tutor.set({
      name: name,
      page: options.page
    }, formatters.set[options.format]);
  });

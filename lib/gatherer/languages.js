...
const paginationSelector = '#ctl00_ctl00_ctl00_MainContent_SubContent_SubContent_languageList_pagingControls';
...
//    languages :: StrMap String -> Promise (StrMap { name :: String, ids :: Array Integer })
module.exports = details =>
  fetch (details) (1)
  .then ($ => Promise.all (U.map (fetch (details))
                                 (U.range (2)
                                          (U.prop ('max') (pagination ($ (paginationSelector))) + 1)))
              .then (U.concat ([$])))
  ...
  .then (rs => U.reduce (map => ({code, id, name}) =>
                           ((map[code] || (map[code] = {name, ids: []})).ids.push (id), map))
                        (Object.create (null))
                        (rs));

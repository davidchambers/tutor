...
module.exports = details =>
  gatherer.request (gatherer.card.url ('Printings.aspx', details))
  .then (extract);
...
const extract = $ => {
  ...
  const legality = {};
  for (let row = U.parent ($ (prefix + '_LegalityList_listRepeater_ctl00_ConditionTableData'));
  ...
  const versions = {};
  for (let row = U.parent (U.parent ($ (prefix + '_PrintingsList_listRepeater_ctl00_cardTitle')));
       row.hasClass ('cardItem');
       row = U.next (row)) {

...
(function() {
  var converted_mana_costs, extract, gatherer, load, supertypes, to_converted_mana_cost, url,
    __slice = [].slice,
  ...
  module.exports = function(name, callback) {
    ...
    ctx = {};
    withParser = function(parse) {
      return function(err, data) {
        var card, clone, clones, id, idx, key, match, value, _ref;
        ...
        if ('set' in ctx && 'basics' in ctx) {
          idx = ctx.set.length;
          while (idx--) {
            card = ctx.set[idx];
            if (card.name in ctx.basics) {
              match = /multiverseid=(\d+)/.exec(card.gatherer_url);
              clones = (function() {
                var _i, _len, _ref, _results;

                _ref = ctx.basics[card.name];
                _results = [];
              ...
              (_ref = ctx.set).splice.apply(_ref, [idx, 1].concat(__slice.call(clones)));
            }
          }
          return callback(null, ctx.set);
        }
      };
    };
    gatherer.request(gatherer.url('/Pages/Search/Default.aspx', {
      ...
      special: true,
      set: "[\"" + name + "\"]"
    }), withParser(function(ctx, body) {
      return ctx.set = extract(body, name);
    }));
    gatherer.request(gatherer.url('/Pages/Search/Default.aspx', {
      set: "[\"" + name + "\"]",
      type: '+["Basic"]+["Land"]'
      ...
      ctx.basics = {};
      return load(body)('.cardItem').each(function() {
        var ids;

        ids = ctx.basics[this.find('.cardTitle').text().trim()] = [];
        return this.find('.setVersions').find('a').filter(function() {
          if (this.children("img[alt^='" + name + "']").length) {
            return ids.push(+url.parse(this.attr('href'), true).query.multiverseid);
          }
        });
      });
    }));

(function() {
  var extract, gatherer, load, request, supertypes, urlmod,
    __slice = [].slice,
  ...
  extract = function(html) {
    ...
    set = {
      ...
      pages: Math.max.apply(Math, [1].concat(__slice.call($('.paging').find('a').map(function() {
        return +urlmod.parse(this.attr('href'), true).query.page + 1;
      })))),
      cards: []
    };

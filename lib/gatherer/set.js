...
(function() {
    ...
    Q.all([d1.promise, d2.promise]).then(function(_arg) {
      ...
      while (idx--) {
        ...
        if (card.name in basics) {
          ...
          [].splice.apply(set, [idx, idx - idx + 1].concat(clones)), clones;
        }
      }
  ...
  extract = function(html, name) {
    ...
    $('.textspoiler').find('tr').each(function() {
      var first, key, param, pattern, power, rarity, second, subtypes, toughness, type, types, val, version, words, _i, _j, _k, _l, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4;
      ...
      switch (key) {
        ...
        case 'Type:':
          _ref1 = /^([^\u2014]+?)(?:\s+\u2014\s+(.+))?$/.exec(val), _i = _ref1.length - 2, types = _ref1[_i++], subtypes = _ref1[_i++];
          _ref2 = types.split(/\s+/);
        ...
        case 'Pow/Tgh:':
          pattern = /^[(]([^\/]*(?:[{][^}]+[}])?)\/([^\/]*(?:[{][^}]+[}])?)[)]$/;
          _ref3 = pattern.exec(val), _k = _ref3.length - 2, power = _ref3[_k++], toughness = _ref3[_k++];

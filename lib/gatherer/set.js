...
(function() {
  ...
  module.exports = function(name, callback) {
    ...
    Q.all([d1.promise, d2.promise]).then(function(_arg) {
      var basics, body1, body2, card, clone, clones, id, idx, key, match, set, value;
      body1 = _arg[0], body2 = _arg[1];
      basics = {};
      load(body2)('.cardItem').each(function() {
        var ids;
        ids = basics[this.find('.cardTitle').text().trim()] = [];
  ...
  extract = function(html, name) {
    ...
    $('.textspoiler').find('tr').each(function() {
      ...
      switch (key) {
        ...
        case 'Cost:':
          return card.converted_mana_cost = to_converted_mana_cost(card.mana_cost = val.replace(/(?:\d+|[^\d(/)])(?![/)])/g, '($&)').replace(/[(]/g, '{').replace(/[)]/g, '}'));
        case 'Type:':

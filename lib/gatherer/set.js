...
module.exports = name => {
  ...
  return gatherer.request (
    gatherer.url ('/Pages/Search/Default.aspx',
                  Object.assign ({page: '0'}, common_params))
  )
  .then (
    $ => {
      const {min, max} = pagination ($ ('#ctl00_ctl00_ctl00_MainContent_SubContent_topPagingControlsContainer'));
      return Promise.all (
        U.map (page =>
                 gatherer.request (
                   gatherer.url ('/Pages/Search/Default.aspx',
                                 Object.assign ({page: String (page)},
                                                common_params))
                 ))
              (U.range (min) (max))
      )
...
const extract_card = set_name => $card => {
  let match;
  const param = `multiverseid=${gatherer._id_from_link (U.find ('a') ($card))}`;
  const card$ = {
    supertypes: [],
    types: [],
    subtypes: [],

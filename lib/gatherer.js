...
const collect_options = label => () =>
  ...
  .then (U.T ('#ctl00_ctl00_MainContent_Content_SearchControls_' + label + 'AddText'))
  .then (U.children)
...
gatherer._get_rules_text = get_text => U.pipe ([
  U.children,
  U.map (get_text),
  U.joinWith ('\n\n'),
]);
...
gatherer._set = (obj, key, value) => {
  if (!(value === undefined || Number.isNaN (value))) obj[key] = value;
};

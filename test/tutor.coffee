...
set = (name, test) -> (done) ->
  ...
  if fs.existsSync path
    wizards.get('/Pages/Search/Default.aspx?output=spoiler&special=true' +
                "&set=[%22#{encodeURIComponent name}%22]")
...
describe 'tutor.set', ->
  ...
  it 'extracts versions',
    set 'Lorwyn', (err, cards) ->
      eq cards[2].name, 'Ajani Goldmane'
      eq cards[2].versions['Lorwyn'].length, 1
      eq cards[2].versions['Lorwyn'][0], 'Rare'
      eq cards[2].versions['Magic 2010'].length, 1
      eq cards[2].versions['Magic 2010'][0], 'Mythic Rare'
      eq cards[2].versions['Magic 2011'].length, 1
      eq cards[2].versions['Magic 2011'][0], 'Mythic Rare'
